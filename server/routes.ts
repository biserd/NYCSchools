...
6. If data isn't available for a specific question, say "I don't have that data" honestly — do NOT make up an answer`;
  ...
  const saveSession = (req: MagicLinkRequest): Promise<void> =>
  ...
  // Complete passwordless login. New clients send the token in a POST body so
  // it does not appear in request URLs or access logs. The GET route below is
  // retained temporarily for links generated by older deployments.
  const verifyMagicLink = async (
    ...
    try {
      ...
      return res
        .set("Cache-Control", "no-store")
        .status(500)
        .json({ error: "Failed to process magic link" });
    }
  };

  app.post("/api/auth/magic-link/verify", async (req: MagicLinkRequest, res: Response) => {
    return verifyMagicLink(req, res, req.body?.token);
  });

  app.get("/api/auth/magic-link/:token", async (req: MagicLinkRequest, res: Response) => {
    return verifyMagicLink(req, res, req.params.token);
  });
  ...
  app.get("/sitemap-legacy.xml", async (req: Request, res: Response) => {
    try {
      ...
      const staticPages = [
        ...
        { url: '/chances-calculator', changefreq: 'monthly', priority: '0.7' },
        ...
        { url: '/safety-methodology', changefreq: 'monthly', priority: '0.6' },
