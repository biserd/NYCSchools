...
export function ParentWhatsAppConnection({ userId }: { userId: string }) {
  ...
  const status = useQuery<{ preview: boolean; configured: boolean; connected: boolean; phone: string | null }>({
  ...
  return <Card><CardHeader><CardTitle>Connect WhatsApp · Preview</CardTitle></CardHeader><CardContent className="space-y-4">
    <p className="text-sm text-muted-foreground">Dedicated NYC School Ratings number: +1 917-473-0386. Link your own WhatsApp phone to your account. This tests account linking and reading your manually entered dates—not AI or automatic reminders. No payment is taken. This preview is limited to 50 incoming requests per day across testers.</p>
    {status.isLoading ? <p role="status">Checking connection…</p> : status.isError ? <p role="alert">Could not check WhatsApp setup.</p> : <>
      ...
      {status.data?.connected ? <><p>Connected: {status.data.phone}. Send <strong>EVENTS</strong> for your next five dates, <strong>STATUS</strong> to check, or <strong>STOP</strong> to disconnect.</p><Button variant="outline" disabled={disconnect.isPending} onClick={() => disconnect.mutate()}>Disconnect WhatsApp</Button></> : <>
        <label className="flex items-start gap-3 text-sm min-h-11"><input className="mt-1 h-5 w-5 shrink-0" type="checkbox" checked={consent} onChange={e => { setConsent(e.target.checked); connect.reset(); }} />I want to link my WhatsApp phone to your account so I can request my family calendar dates. I can disconnect here or send STOP. This does not opt me into marketing.</label>
