...
export function ParentAssistantPanel({userId,events,onSaved}:{userId:string;events:TuckOverview['events'];onSaved:()=>void}) {
  ...
  return <section className="space-y-6" aria-label="Parent Assistant">
    <Card className="border-teal-300"><CardHeader><CardTitle>Parent Assistant</CardTitle></CardHeader><CardContent className="space-y-5">
      ...
      <form className="space-y-3" onSubmit={e=>{e.preventDefault();setAnswer(null);action.mutate({path:'/message',body:{message}});}}>
        ...
        <Button disabled={busy||!prefs.aiConsent}>Ask Parent Assistant</Button>
        <p className="text-xs text-muted-foreground">Up to {status.data?.limits.questionsPerDay} requests/day. AI interprets requests; school statistics come from our database. Avoid sensitive child, medical or financial details.</p>
      </form>
    ...
    <Card><CardHeader><CardTitle>Your privacy &amp; reminder preferences</CardTitle></CardHeader><CardContent>
      <form className="space-y-4" onSubmit={e=>{e.preventDefault();action.mutate({path:'/preferences',method:'PUT',body:prefs});}}>
        ...
        <label className="flex gap-3 min-h-11 items-start"><input type="checkbox" className="h-5 w-5 mt-1 shrink-0" checked={prefs.reminderConsent} onChange={e=>setPrefs({...prefs,reminderConsent:e.target.checked})}/><span>I want WhatsApp reminders for events I choose. Up to {status.data?.limits.remindersPerMonth} reminders per UTC calendar month. Send STOP or disconnect to revoke consent and cancel pending reminders. No marketing.</span></label>
        <label className="flex gap-3 min-h-11 items-start"><input type="checkbox" className="h-5 w-5 mt-1 shrink-0" checked={prefs.aiConsent} onChange={e=>setPrefs({...prefs,aiConsent:e.target.checked})}/><span>Allow my assistant requests to be processed by Cloudflare AI. My saved family records are not sent to the model. Drafts expire after 10 minutes.</span></label>
    ...
    <Card><CardHeader><CardTitle>WhatsApp reminders</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      <form className="space-y-3" onSubmit={e=>{e.preventDefault();action.mutate({path:'/reminders',body:{eventId,localDate,localTime}});}}>
        ...
        <Button disabled={busy||!status.data?.preferences.reminderConsent}>Schedule reminder</Button>
      </form>
