  ...
  const addToComparison = useCallback((school: School): { success: boolean; error?: string } => {
    if (comparedSchools.length >= maxCompare) {
      const message = isPremium 
        ...
        : `Free accounts can compare up to ${FREE_MAX_COMPARE} schools. Family Premium increases the limit to ${PREMIUM_MAX_COMPARE}; existing paid plans keep their benefits.`;
