...
import {localInstant,quietNow,savePreferences,hasFamilyAccess,createReminder,cancelReminder,assistantOverview,type AssistantEnvironment} from '../../server/parent/service';
...
try {
  ...
  await DB.prepare("INSERT INTO users(id,email,password,subscription_status,subscription_plan,subscription_expires_at) VALUES ('a','a@example.invalid','test','active','season_pass',?),('b','b@example.invalid','test','free','free',NULL)").bind(Date.now()+86400000).run();
  ...
  assert.equal(await hasFamilyAccess('a',{...env,ENVIRONMENT:'production'}),false,'Research Pass cannot grant assistant access');
  ...
  assert.equal(await hasFamilyAccess('a',{...env,ENVIRONMENT:'production'}),true,'Cancel-at-period-end retains access');
  ...
  await Promise.all([familyCheckout('a',checkoutEnv,stripe),familyCheckout('a',checkoutEnv,stripe)]);
