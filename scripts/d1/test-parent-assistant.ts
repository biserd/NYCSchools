...
try {
  ...
  const conversationEnv={...env,AI:{run:async()=>({choices:[{message:{content:"That sounds overwhelming. Let's take it one decision at a time—what matters most to your family right now?"}}]})} as Ai};
  const supportive=await answerParent('a',conversationEnv,{message:'I feel overwhelmed by this whole school search'},async()=>plan({action:'conversation'}));
