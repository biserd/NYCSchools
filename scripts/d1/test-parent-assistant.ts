...
import {answerParent,confirmDraft,stageDraft,suggestCalendarEvent,parentAgentModel,withCurrentSchoolContext,conversationalParentReply,requiresParentTaskPlanning,parseIntent} from '../../server/parent/assistant';
...
try {
  ...
  const usageBeforeSocial=Number(await DB.prepare("SELECT coalesce(sum(count),0) n FROM parent_usage WHERE user_id='a'").first('n'));
  const hello=await answerParent('a',env,{message:'Hi'});
  assert.match(hello.message,/Hi!/);
  const reaction=await answerParent('a',env,{message:'Wow'});
  assert.match(reaction.message,/I’m with you/);
  const naturalChat=await answerParent('a',env,{message:'Can I chat with you like a person?'});
  assert.match(naturalChat.message,/talk to me normally/i);
  assert.match(naturalChat.message,/AI assistant, not a human/i,'Personable behavior must not falsely claim human identity');
  assert.equal(conversationalParentReply('What day is today?',null,Date.parse('2026-09-20T16:00:00Z'),'America/New_York'),'Today is Sunday, September 20, 2026.');
  const today=await answerParent('a',env,{message:'What day is today?'});
  assert.match(today.message,/Sunday, September 20, 2026|Today is/,'Everyday date questions bypass the planner');
  assert.equal(requiresParentTaskPlanning('What day is today?'),false);
  assert.equal(requiresParentTaskPlanning('Which elementary schools are safest on the UWS?'),true);
  const plannerEnv={...env,AI:{run:async(model:unknown,input:unknown)=>{
    assert.equal(model,PARENT_AGENT_FAST_MODEL);
    assert.deepEqual((input as {response_format:{json_schema:unknown}}).response_format.json_schema.type,'object','Planner uses Workers AI JSON schema directly');
    return {response:{action:'school_search',schoolQuery:null,schoolQueries:[],locationKind:'district',locationValue:'2',gradeLevel:'elementary',programs:[],sort:'overall',title:null,date:null,reminderDate:null,reminderTime:null,clarification:null}};
  }} as Ai};
  const parsedPlanner=await parseIntent(plannerEnv,'Show the top elementary schools in district 2','2026-09-20','America/New_York');
  assert.equal((parsedPlanner as AgentPlan).location?.value,'2','Planner accepts the native Workers AI response object');
  const usageAfterSocial=Number(await DB.prepare("SELECT coalesce(sum(count),0) n FROM parent_usage WHERE user_id='a'").first('n'));
  assert.equal(usageAfterSocial,usageBeforeSocial,'Greetings and reactions do not consume the daily grounded-question allowance');
  let conversationCalls=0;
  const conversationEnv={...env,AI:{run:async(model:unknown)=>{conversationCalls++;assert.equal(model,PARENT_AGENT_CONVERSATION_MODEL);return {response:"That sounds overwhelming. Let's take it one decision at a time—what matters most to your family right now?"};}} as Ai};
  const supportive=await answerParent('a',conversationEnv,{message:'I feel overwhelmed by this whole school search'});
  assert.match(supportive.message,/one decision at a time/,'General parent discussion uses the conversational response path');
  assert.equal(conversationCalls,1,'General conversation uses one small-model call and never the structured planner');
  ...
  assert.match(malformed.message,/didn’t quite understand/);
