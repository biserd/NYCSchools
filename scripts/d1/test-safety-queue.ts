...
globalThis.fetch=async(input,init)=>{
 if(String(input).startsWith('https://data.cityofnewyork.us/')){requests++;if(failNext){failNext=false;return new Response('simulated source failure',{status:503});}return Response.json(fixtures);}
 return originalFetch(input,init);
};
try{
 for(const file of ['0000_harsh_johnny_blaze.sql','0001_sturdy_gabe_jones.sql']){
 ...
 await env.DB.prepare("INSERT INTO schools(dbn,name,district,address,grade_band,latitude,longitude) VALUES('99T000','Synthetic school',1,'Synthetic','K-5',40.7,-74)").run();
 await withDatabaseInstance(createDatabase(env.DB),async()=>{
  ...
  let runs=0;while(messages.length){assert.ok(runs++<10,'Queue loop did not terminate');await consumeSafetyRefresh(delivery(messages.shift()),env);}
  ...
  assert.equal(results.length,4);for(const row of results){assert.equal(row.total_reports,1);assert.equal(row.felony_reports,1);assert.equal(row.violent_felony_reports,1);assert.equal(row.prior_period_total,1);assert.equal(row.safety_index,100);}
  ...
  const report={passed:['source failure retries without advancing checkpoint','two-dataset complaint deduplication','queued phase transitions','all four safety radii','current/prior date boundaries','violent felony classification','percentile finalization','completed-job duplicate ignored'],requests,queueMessages:sent.length};
