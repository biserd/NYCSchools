...
- **Neighborhood Safety Index**: 0-100 parent-friendly safety score on every public, private, and NYCEEC school detail page, calculated from NYPD complaint data (Socrata datasets `5uac-w243` YTD + `qgea-i56i` historic) within four radius options (0.25/0.5/1/5 miles, default 0.5mi). Severity-weighted (violent felony 8 / felony 4 / misdemeanor 2 / violation 1) over a rolling 12-month window, ranked as a citywide percentile so 50 = average. Free tier shows score + label at the default radius; Premium unlocks radius selector, top categories, trend (improving / stable / worsening vs prior 12 months), and percentile vs other NYC schools. Tables: `nypd_complaints`, `school_safety_index`. Sync script: `npx tsx scripts/sync-safety-index.ts [--recompute] [--months=N] [--max-rows=N]` (run monthly). Public methodology page at `/safety-methodology`.
